...
class WorkShop:
    ...
    def getReviews(self, urid, offset=0, limit=10):
    ...
    def replyReview(self, text, urid, messageid, parent=0):
        ...
        self.i = 0
        while (self.statusCode != 200 and self.statusCode != 201) and self.i < 4:
    ...
    def getAllWorks(self, urid):
        ...
        for self.i in range(self.n + 1):
            ...
            for self.j in self.r:
                self.end.append({"workId": self.j["id"], "workName": self.j["name"], "creatorId": self.j["user"]["id"],
                                 "creatorName": self.j["user"]["nickname"]})
